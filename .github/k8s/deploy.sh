...
deploy_status=0
if [ "${DEPLOY_MODE:-false}" = "false" ]; then
  exit "$deploy_status"
fi

if ! (uv pip install --system huggingface_hub && python upload.py "${GITHUB_REF_NAME}"); then
...
if ! tar -czf "$archive_file" examples/${MAJOR_VERSION}; then
